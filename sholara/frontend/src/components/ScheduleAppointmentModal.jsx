...
const ScheduleAppointmentModal = ({ faculty, onClose, onSuccess }) => {
  ...
  // Fetch available slots whenever faculty or selected date changes
  useEffect(() => {
    const fetchSlots = async () => {
      ...
      try {
        ...
        const res = await axios.get(
          `/api/appointments/slots/${faculty._id}?date=${selectedDate}`,
    ...
    <div className="fixed inset-0 bg-black/50 flex items-center justify-center p-4 z-50">
      <div className="bg-white rounded-lg shadow-xl max-w-md w-full p-6 relative">
        ...
        <p className="text-sm text-gray-600 mb-4">
          Faculty Member: <span className="font-semibold">{faculty?.name}</span>
        </p>
